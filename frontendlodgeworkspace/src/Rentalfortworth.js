...
import { faDollarSign, faHome, faMapMarkerAlt, faUser } from '@fortawesome/free-solid-svg-icons';
...
export default class Rentalfortworth extends React.Component {
    ...
    isDateValid = () => {
        ...
        if(this.state.beginDate.getTime() > this.state.currentDate.getTime() && this.state.endDate.getTime() > this.state.beginDate.getTime()) {
            if(((this.state.endDate.getTime() - this.state.beginDate.getTime())/(1000*3600*24)) <= this.props.clickedRental.price) {
                this.onFees()
                this.onBookedRental()
            }
            else {
                alert("Please choose a check-in and check-out date where the days between is equal or less than the days next to the clock.")
            }
        }
    }
    addRental = () => {
            ...
            <div className="div-rfm">
                ...
                <div className="rfm-div2">
                    ...
                    <div className="rdv2-div3">
                        <p>
                            The Lone Star State very own Fort Worth has so much to offer. 
                            You can take a tour of the famous and historic Fort Worth Stockyards. 
                            Whether it heart-racing or just racing, the Texas Motor Speedway 
                            got you covered. With so many scenic and vibrant spots, Fort Worth will 
